# -*- coding: utf-8 -*-
"""설정된 텔레그램 대화에서 메시지를 추출해 Report/raw 에 저장합니다."""
...
async def resolve_entity(client, spec: dict, dialogs_by_title: dict):
    ...
    if spec.get("title_contains"):
        ...
        raise ValueError(f"title_contains '{spec['title_contains']}' 에 맞는 대화를 찾지 못함")
...
async def main() -> None:
    cfg = load_config()
    since = since_dt(cfg)
    out_dir = Path(cfg.get("output_dir", ROOT / "raw"))
    ...
    prompt = f"""아래는 텔레그램 시황 원문입니다.
기관투자자용 채권 모닝브리핑 PDF를 이 폴더(C:\\Users\\infomax\\Documents\\Report)에 저장해 주세요.
